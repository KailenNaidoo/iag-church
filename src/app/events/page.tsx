export default function EventsPage() {
  const events = [
    ...
    },
    {
      ...
      description: "Games, worship, and teaching for young people aged 13–25.",
      recurring: true,
    },
    {
      ...
      location: "Main Sanctuary",
      description: "A dedicated week of corporate prayer and fasting for breakthrough.",
      recurring: false,
    },
    {
      ...
      description: "Serving our community through food distribution, prayer, and fellowship.",
      recurring: false,
    },
    {
      ...
      time: "All Day",
      location: "Main Sanctuary",
      description: "Three days of powerful ministry, guest speakers, and spiritual renewal.",
      recurring: false,
    },
    {
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {recurringEvents.map((event) => (
              <div key={event.title} className="p-8 border border-[var(--border)] bg-[var(--card-bg)] hover:border-[var(--gold)]/40 transition-all duration-500">
                <h3 className="font-serif text-lg mb-4">{event.title}</h3>
                <div className="space-y-2 text-sm text-[var(--muted)]">
                  <p className="flex items-center gap-2">
                    <span className="text-[var(--gold)]">◆</span> {event.date}
                  </p>
                  <p className="flex items-center gap-2">
                    <span className="text-[var(--gold)]">◆</span> {event.time}
                  </p>
                  <p className="flex items-center gap-2">
                    <span className="text-[var(--gold)]">◆</span> {event.location}
                  </p>
                </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-6">
            {upcomingEvents.map((event) => (
              <div
                key={event.title}
                className="p-8 border border-[var(--border)] bg-[var(--background)] hover:border-[var(--gold)]/40 transition-all duration-500 flex flex-col md:flex-row md:items-center gap-6"
              >
